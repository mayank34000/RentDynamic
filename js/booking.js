...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchListings() {
        ...
        // Exclude blocked listings — they must not appear in Browse Rentals
        allListings = allListings.filter(l => (l.status || '').toLowerCase() !== 'blocked');
    }
    ...
    function handleBookingSubmit(e) {
        ...
        // Re-read from storage — listing may have been blocked after the page loaded
        try {
            const freshAll = JSON.parse(localStorage.getItem('RentFlow_listings')) || [];
            const freshListing = freshAll.find(l => l.id === activeBookingListingId);
            if (freshListing && (freshListing.status || '').toLowerCase() === 'blocked') {
                showToast('This listing is currently unavailable.', 'error');
